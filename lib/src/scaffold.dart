...
class ScaffoldMessengerState extends State<ScaffoldMessenger> with TickerProviderStateMixin {
  ...
  ScaffoldFeatureController<SnackBar, SnackBarClosedReason> showSnackBar(SnackBar snackBar) {
    _snackBarController ??= SnackBar.createAnimationController(vsync: this)..addStatusListener(_handleStatusChanged);
...
///
/// {@tool dartpad --template=stateful_widget_material}
/// This example shows a [Scaffold] with an [FluentAppBar], a [BottomAppBar] and a
...
///
/// {@tool snippet}
/// Add a listener to the app's tab controller so that the [FluentAppBar] title of the
...
/// See also:
///
///  * [FluentAppBar], which is a horizontal bar typically shown at the top of an app
...
class Scaffold extends StatefulWidget {
  ...
  ///
  /// This is useful if the app bar's [FluentAppBar.backgroundColor] is not
  /// completely opaque.
  ///
...
class ScaffoldState extends State<Scaffold> with TickerProviderStateMixin, RestorationMixin {
  ...
  void openDrawer() {
    if (_endDrawerKey.currentState != null && _endDrawerOpened.value) _endDrawerKey.currentState!.close();
  ...
  /// See [Scaffold.of] for information about how to obtain the [ScaffoldState].
  void openEndDrawer() {
    if (_drawerKey.currentState != null && _drawerOpened.value) _drawerKey.currentState!.close();
  ...
  void _updateSnackBar() {
    setState(() {
      _messengerSnackBar = _scaffoldMessenger!._snackBars.isNotEmpty ? _scaffoldMessenger!._snackBars.first : null;
    });
  }
  ...
  void didChangeDependencies() {
    ...
    if (_scaffoldMessenger != null && (_currentScaffoldMessenger == null || _scaffoldMessenger != _currentScaffoldMessenger)) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_snackBars.isNotEmpty) {
      final SnackBarBehavior snackBarBehavior =
          _snackBars.first._widget.behavior ?? themeData.snackBarTheme.behavior ?? SnackBarBehavior.fixed;
      isSnackBarFloating = snackBarBehavior == SnackBarBehavior.floating;
