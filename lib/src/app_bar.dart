...
/// app bar, see [SliverAppBar], which embeds an [FluentAppBar] in a sliver for use in
/// a [CustomScrollView].
///
...
/// English):
///
/// The [FluentAppBar] insets its content based on the ambient [MediaQuery]'s padding,
/// to avoid system UI intrusions. It's taken care of by [Scaffold] when used in
/// the [Scaffold.appBar] property. When animating an [FluentAppBar], unexpected
/// [MediaQuery] changes (as is common in [Hero] animations) may cause the content
/// to suddenly jump. Wrap the [FluentAppBar] in a [MediaQuery] widget, and adjust its
...
///
/// If the [leading] widget is omitted, but the [FluentAppBar] is in a [Scaffold] with
...
///
/// This sample shows an [FluentAppBar] with two simple actions. The first action
...
/// See also:
///
///  * [Scaffold], which displays the [FluentAppBar] in its [Scaffold.appBar] slot.
///  * [SliverAppBar], which uses [FluentAppBar] to provide a flexible app bar that
///    can be used in a [CustomScrollView].
///  * [TabBar], which is typically placed in the [bottom] slot of the [FluentAppBar]
...
class FluentAppBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  FluentAppBar({
    ...
    this.title,
    this.openDrawerOnLeadingTap = false,
    this.actions,
    this.flexibleSpace,
    this.bottom,
    this.elevation,
    ...
    this.systemOverlayStyle,
        ...
        assert(openDrawerOnLeadingTap != null),
        assert(primary != null),
  ...
  /// [FluentAppBar] will imply an appropriate widget. For example, if the [FluentAppBar] is
  /// in a [Scaffold] that also has a [Drawer], the [Scaffold] will fill this
  /// widget with an [IconButton] that opens the drawer (using [Icons.menu]). If
  /// there's no [Drawer] and the parent [Navigator] can go back, the [FluentAppBar]
  ...
  /// See also:
  ///
  ///  * [Scaffold.appBar], in which an [FluentAppBar] is usually placed.
  ...
  /// Controls whether to open drawer when leading is pressed. default value is false.
  ///
  /// If true and [Drawer] does not exist in scaffold it has no effect.
  /// If false and ir will have no effect.
  /// If [Drawer] does exists in scaffold and this is true it will open [Drawer].
  /// {@endtemplate}
  final bool openDrawerOnLeadingTap;
  ...
  ///
  /// A flexible space isn't actually flexible unless the [FluentAppBar]'s container
  /// changes the [FluentAppBar]'s size. A [SliverAppBar] in a [CustomScrollView]
  /// changes the [FluentAppBar]'s height when scrolled.
  ///
  ...
  /// null, then [FluentAppBar] uses the overall theme's [ColorScheme.primary] if the
  ...
  /// value is also null, then [FluentAppBar] uses the overall theme's
  ...
  /// Defines the height of the toolbar component of an [FluentAppBar].
  ///
...
class _FluentAppBarState extends State<FluentAppBar> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.toolbarOpacity != 1.0) {
      ...
      if (titleTextStyle?.color != null) titleTextStyle = titleTextStyle!.copyWith(color: titleTextStyle.color!.withOpacity(opacity));
    ...
    if (leading != null && !widget.openDrawerOnLeadingTap) {
    ...
    return Semantics(
      ...
      child: AnnotatedRegion<SystemUiOverlayStyle>(
        ...
        child: Material(
          ...
          shadowColor: widget.shadowColor ?? appBarTheme.shadowColor ?? _defaultShadowColor,
          shape: widget.shape,
          child: Semantics(
            explicitChildNodes: true,
            child: appBar,
...
/// the screen see [FluentAppBar], which is used in the [Scaffold.appBar] slot.
///
...
///    scrolling.
///  * [FluentAppBar], which is a fixed-height app bar for use in [Scaffold.appBar].
///  * [TabBar], which is typically placed in the [bottom] slot of the [FluentAppBar]
///    if the screen has multiple pages arranged in tabs.
///  * [IconButton], which is used with [actions] to show buttons on the app bar.
///  * [PopupMenuButton], to show a popup menu on the app bar, via [actions].
...
class SliverAppBar extends StatefulWidget {
  ...
  ///
  /// This property is used to configure an [FluentAppBar].
  final Widget? leading;
  ...
  /// content is not scrolled under the [FluentAppBar].
  ///
  /// Defaults to false, meaning that the [elevation] is only applied when the
  /// [FluentAppBar] is being displayed over content that is scrolled under it.
  ///
